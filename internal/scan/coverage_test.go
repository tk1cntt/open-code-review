...
func TestExecuteSubtask_Success(t *testing.T) {
	...
	completed, _, err := a.executeSubtask(context.Background(), it, session.FileStart{})
	if err != nil {
...
func TestExecuteSubtask_ContextCancelled(t *testing.T) {
	...
	_, _, err := a.executeSubtask(ctx, model.ScanItem{Path: "a.go", Content: "x", LineCount: 1}, session.FileStart{})
	if err == nil {
