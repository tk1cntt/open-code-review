...
func TestParseReviewFlags_Effort(t *testing.T) {
	opts, err := parseReviewFlags([]string{"--effort", "high"})
	if err != nil {
		t.Fatalf("parseReviewFlags: %v", err)
	}
	if opts.effort != "high" {
	...
	if _, err := parseReviewFlags([]string{"--effort", "extreme"}); err == nil {
		t.Fatal("expected error for invalid --effort")
	}
}

func TestParseReviewFlags_NoFilter(t *testing.T) {
	opts, err := parseReviewFlags([]string{"--no-filter"})
	if err != nil {
		t.Fatalf("parseReviewFlags: %v", err)
	}
	if !opts.noFilter {
		t.Error("expected noFilter=true")
	}
}

func TestParseReviewFlags_Apply(t *testing.T) {
	opts, err := parseReviewFlags([]string{"--apply"})
	if err != nil {
		t.Fatalf("parseReviewFlags: %v", err)
	}
	if !opts.apply {
...
func TestParseReviewFlags_ResumeMode(t *testing.T) {
	opts, err := parseReviewFlags([]string{"--from", "main", "--to", "feature", "--resume", "s1", "--resume-mode", "restart-failed"})
	if err != nil {
		t.Fatalf("parseReviewFlags: %v", err)
	}
	if opts.resumeMode != "restart-failed" {
		t.Errorf("resumeMode = %q, want restart-failed", opts.resumeMode)
	}
	if _, err := parseReviewFlags([]string{"--resume-mode", "bogus"}); err == nil {
		t.Fatal("expected error for invalid --resume-mode")
	}
}
