...
type mockResultProvider struct {
	...
	fileGroups       []agent.FileGroupInfo
}
...
func (m *mockResultProvider) FileGroups() []agent.FileGroupInfo { return m.fileGroups }
...
func TestEmitRunResult_JSONNoFiles(t *testing.T) {
	...
	got := captureStdout(t, func() {
		err := emitRunResult(context.Background(), ag, nil, time.Second, "json", "developer", nil, identity, os.Stdout, nil)
		if err != nil {
...
func TestEmitRunResult_JSONLLMIdentityNamedProvider(t *testing.T) {
	...
	identity := &jsonLLMIdentity{Provider: "anthropic", Model: "claude-opus-4-6"}
	got := captureStdout(t, func() {
		if err := emitRunResult(context.Background(), ag, nil, time.Second, "json", "developer", nil, identity, os.Stdout, nil); err != nil {
...
func TestEmitRunResult_JSONUsesManifestTerminalState(t *testing.T) {
	...
	got := captureStdout(t, func() {
		if err := emitRunResult(context.Background(), ag, nil, time.Second, "json", "developer", nil, nil, os.Stdout, nil); err != nil {
			t.Fatalf("emitRunResult: %v", err)
		}
	})
	var out jsonOutput
...
func TestEmitRunResult_JSONWithComments(t *testing.T) {
	...
	got := captureStdout(t, func() {
		err := emitRunResult(context.Background(), ag, comments, time.Second, "json", "developer", nil, nil, os.Stdout, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
...
func TestEmitRunResult_JSONWithResumeInfo(t *testing.T) {
	...
	got := captureStdout(t, func() {
		err := emitRunResult(context.Background(), ag, nil, time.Second, "json", "developer", nil, nil, os.Stdout, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
...
func TestEmitRunResult_TextNoComments(t *testing.T) {
	...
	got := captureStdout(t, func() {
		err := emitRunResult(context.Background(), ag, nil, time.Second, "text", "developer", nil, nil, os.Stdout, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
...
func TestEmitRunResult_TextPartialNeverLooksGood(t *testing.T) {
	...
	got := captureStdout(t, func() {
		if err := emitRunResult(context.Background(), ag, nil, time.Second, "text", "developer", nil, nil, os.Stdout, nil); err != nil {
			t.Fatalf("emitRunResult: %v", err)
		}
	})
...
func TestEmitRunResult_TextCompleteReportsFindingsAndWaived(t *testing.T) {
	...
	got := captureStdout(t, func() {
		if err := emitRunResult(context.Background(), ag, comments, time.Second, "text", "developer", nil, nil, os.Stdout, nil); err != nil {
			t.Fatalf("emitRunResult: %v", err)
		}
	})
...
func TestEmitRunResult_TextWithComments(t *testing.T) {
	...
	got := captureStdout(t, func() {
		err := emitRunResult(context.Background(), ag, comments, time.Second, "text", "developer", nil, nil, os.Stdout, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
...
func TestEmitRunResult_AgentTextRestoresQuiet(t *testing.T) {
	...
	got := captureStdout(t, func() {
		err := emitRunResult(context.Background(), ag, nil, time.Second, "text", "agent", q, nil, os.Stdout, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
...
func TestEmitRunResult_AgentJSONDoesNotRestore(t *testing.T) {
	...
	got := captureStdout(t, func() {
		err := emitRunResult(context.Background(), ag, nil, time.Second, "json", "agent", q, nil, os.Stdout, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
...
func TestEmitRunResult_NilQuietHandle(t *testing.T) {
	ag := &mockResultProvider{filesReviewed: 1}
	got := captureStdout(t, func() {
		err := emitRunResult(context.Background(), ag, nil, time.Second, "text", "agent", nil, nil, os.Stdout, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
...
func TestEmitRunResult_JSONTraceIDFromContext(t *testing.T) {
	...
	got := captureStdout(t, func() {
		err := emitRunResult(ctx, ag, nil, time.Second, "json", "developer", nil, nil, os.Stdout, nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
...
func TestEmitRunResult_JSONIncludesFileGroups(t *testing.T) {
	ag := &mockResultProvider{
		filesReviewed: 2,
		fileGroups: []agent.FileGroupInfo{
			{Label: "auth", Files: []string{"a.go", "b.go"}},
		},
	}
	got := captureStdout(t, func() {
		if err := emitRunResult(context.Background(), ag, nil, time.Second, "json", "developer", nil, nil, os.Stdout, nil); err != nil {
	...
	if len(out.Groups) != 1 || out.Groups[0].Label != "auth" || len(out.Groups[0].Files) != 2 {
...
func TestEmitRunResult_JSONCarriesRetryReport(t *testing.T) {
	...
	got := captureStdout(t, func() {
		if err := emitRunResult(context.Background(), ag, nil, time.Second, "json", "developer", nil, nil, os.Stdout, rep); err != nil {
			t.Fatalf("emitRunResult: %v", err)
		}
	})
	var out jsonOutput
...
func TestEmitRunResult_TextReportOrder(t *testing.T) {
	...
	got := captureStdout(t, func() {
		if err := emitRunResult(context.Background(), ag, nil, time.Second, "text", "developer", nil, nil, os.Stdout, retryReportFixture()); err != nil {
			t.Fatalf("emitRunResult: %v", err)
		}
	})
...
func TestEmitRunResult_JSONHasNoReportText(t *testing.T) {
	ag := &mockResultProvider{filesReviewed: 2, manifest: mockManifest(session.StateComplete)}
	got := captureStdout(t, func() {
		if err := emitRunResult(context.Background(), ag, nil, time.Second, "json", "developer", nil, nil, os.Stdout, retryReportFixture()); err != nil {
			t.Fatalf("emitRunResult: %v", err)
		}
	})
	if strings.Contains(got, "LLM retry report summary:") {
