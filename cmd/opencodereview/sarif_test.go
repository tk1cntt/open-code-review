...
func TestEmitRunResult_SarifNoFiles(t *testing.T) {
	...
	out := captureStdout(t, func() {
		if err := emitRunResult(context.Background(), ag, nil, time.Second, "sarif", "developer", nil, nil, os.Stdout, nil); err != nil {
...
func TestEmitRunResult_Sarif(t *testing.T) {
	...
	out := captureStdout(t, func() {
		if err := emitRunResult(context.Background(), ag, comments, time.Second, "sarif", "developer", nil, nil, os.Stdout, nil); err != nil {
			t.Fatalf("emitRunResult: %v", err)
		}
	})
	doc := mustUnmarshal(t, out)
